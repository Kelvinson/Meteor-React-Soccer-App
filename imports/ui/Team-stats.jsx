...
export default class TeamStats extends Component {
  render() {
    ...
    // const team = get a total for ballManipulaition
    // const possible = 3 * numPlayers

    const ballManipulaition = Math.round((players.reduce((ballManipulaition,player)=>{
      return ballManipulaition + player.ballManipulaition;
    },0) /(3 * numPlayers))*100);
    ...
    const defense = Math.round((dualTackling + fieldCoverage + blockingAbilities + gameStrategy + playmakingRisks)/5);
    const offense = Math.round((kickingAbilities + ballManipulaition + passingAbilities + fieldCoverage +gameStrategy+playmakingRisks)/6);
    const total = Math.round((kickingAbilities + ballManipulaition + passingAbilities + fieldCoverage +gameStrategy+playmakingRisks+blockingAbilities+dualTackling)/8);
    const data = {
      labels: ['Ball Manipulaition', 'Kicking', 'Passing', 'Duel/Tackling', 'Field Coverage', 'Blocking', 'Strategy','Risks'],
      datasets: [
        {
          ...
          data: [ballManipulaition, kickingAbilities, passingAbilities, dualTackling, fieldCoverage, blockingAbilities, gameStrategy,playmakingRisks]
        }
      ]
    };
    return (
